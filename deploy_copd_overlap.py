slug = "best-mattress-copd-overlap"
...
# 2. CATEGORIES -- Health Conditions (after best-mattress-kidney-disease)
with open("generate_posts_index.py", "r", encoding="utf-8") as f:
    gen = f.read()

if slug not in gen:
    gen = gen.replace(
        "'best-mattress-kidney-disease'",
        "'best-mattress-kidney-disease', 'best-mattress-copd-overlap'"
    )
...
card_check = 'href="posts/best-mattress-copd-overlap.html"'
...
card = (
    ...
    '          <h3><a href="posts/best-mattress-copd-overlap.html">Best Mattress for COPD &amp; Sleep Apnea Overlap Syndrome</a></h3>\n'
    '          <p>7 picks for COPD-OSA overlap &mdash; head elevation, PAP therapy compatibility, oxygen positioning &amp; hypoxic arousal reduction.</p>\n'
