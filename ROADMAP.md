# SleepWise Project Roadmap

## Current Status: Phase 1 Complete - Phase 2 Ready
**Last Updated:** March 10, 2025

---

## Phase 1: Foundation & Content
**Status: COMPLETE**

### What's Done:
- [x] Website live at https://hanysharaf.github.io/sleepwisereviews/
- [x] 26 articles published (2020-2025 history)
- [x] Sitemap with 32 URLs submitted to Google Search Console
- [x] Sitemap checker workflow (daily Telegram status)
- [x] Telegram bot sends daily tips
- [x] Telegram bot sends Instagram content
- [x] Telegram bot sends engagement reminders
- [x] Content queue with 21 tips, 12 IG posts
- [x] GitHub Actions runs every 2 hours
- [x] Amazon affiliate links ready (tag: sleepwiserevi-20)
- [x] Pinterest account set up
- [x] Instagram account set up
- [x] Weekly content folders created
- [x] Archive section on website
- [x] First engagement: 205K verified account reposting content!

### Your Daily Job:
- [ ] Post 1 Pinterest pin (from content folder)
- [ ] Post 1 Instagram post (from IG content)
- [ ] 5 min engagement

---

## Phase 2: Video/Reel Creation
**Status: READY TO START**

### What it will do:
- Auto-generate video from daily tip
- Send video to Telegram
- You download and post

### Tools needed:
- n8n (free, self-hosted)
- Creatomate or Shotstack (video API)
- ~$12/month for video generation

### When to start:
When you're ready for video content on Instagram/Pinterest

---

## Phase 3: Auto-Posting (Future)
**Status: PLANNED - Not started**

### What it will do:
- Auto-post to Instagram (no manual work)
- Auto-post to Pinterest
- Full automation

### Tools needed:
- Meta Business Suite setup
- Instagram Graph API
- n8n workflows

### When to start:
After Phase 2 is working

---

## Completed Milestones

| Milestone | Status | Date |
|-----------|--------|------|
| Website Launch | Done | March 2025 |
| 26 Articles Published | Done | March 2025 |
| Pinterest Setup | Done | March 2025 |
| Instagram Setup | Done | March 2025 |
| Telegram Bot | Done | March 2025 |
| GitHub Actions Automation | Done | March 2025 |
| Weekly Content System | Done | March 2025 |
| Sitemap (32 URLs) Submitted | Done | March 10, 2025 |
| Sitemap Checker Workflow | Done | March 10, 2025 |
| First Big Engagement (205K account) | Done | March 10, 2025 |

---

## Future Ideas (Not Priority Now)

| Idea | Priority | Notes |
|------|----------|-------|
| TikTok | Low | Master IG/Pinterest first |
| YouTube | Low | Too much effort now |
| Email list | Medium | Need audience first |
| Custom domain | Medium | When traffic grows |
| More product reviews | Medium | Based on affiliate data |

---

## Success Metrics

### Phase 1 Goals (Current):
- [ ] 14 Pinterest pins posted
- [ ] 14 Instagram posts
- [ ] 100 Pinterest followers
- [ ] 50 Instagram followers

### Month 1 Goals:
- [ ] 30 Pinterest pins
- [ ] 30 Instagram posts
- [ ] First affiliate sale!

### Month 3 Goals:
- [ ] 500 Pinterest followers
- [ ] 200 Instagram followers
- [ ] 5 affiliate sales/month
- [ ] Website getting organic traffic

---

## Analytics Focus

Based on Pinterest data:
- **Weighted Blankets** = 52% of impressions (focus here!)
- **"Best X for Sleep"** format works well
- **Product-focused pins** get more outbound clicks

### Strategy:
1. Create more weighted blanket content
2. Use "Best [Product] for Sleep" titles
3. Include clear Amazon CTA
4. Track which articles get traffic

---

## Questions to Answer Before Next Phase

Before Phase 2 (video automation):
1. Is manual posting taking too much time?
2. Are videos getting better engagement than images?
3. Do you have budget for video API (~$12/mo)?

Before Phase 3 (auto-posting):
1. Is content performing well?
2. Ready to set up Facebook Business?
3. Consistent posting for 1+ month?

---

## Links

- **Website:** https://hanysharaf.github.io/sleepwisereviews/
- **Pinterest:** https://www.pinterest.com/sleepwisereviews
- **Instagram:** https://www.instagram.com/sleepwise.reviews
- **Affiliate Tag:** sleepwiserevi-20

---

## Rule: Consistency Over Features!

Focus on daily posting for 2 weeks before adding new features.
